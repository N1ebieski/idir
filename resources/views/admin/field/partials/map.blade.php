...
<div class="form-group">
    <label for="field.{{ $field->id }}">
        ...
        @endif
        <span>
            <a id="remove-marker" href="#" class="badge badge-primary"
            {{ !isset($value) || count($value) === 0 ? 'style=display:none' : null }}>             
                {{ trans('idir::fields.remove_marker') }}
            </a>
        </span>        
        <span>
            <a id="add-marker" href="#" class="badge badge-primary"
            {{ isset($value) && count($value) > 0 ? 'style=display:none' : null }}>            
    ...
    @render('idir::map.dir.mapComponent', [
        // 'coords_marker_pattern' => $value,
        'selector' => 'map-select',
        'zoom' => 8
    ])
    <div id="marker0">
        <div>
            <input type="hidden" id="field.{{ $field->id }}.0.lat" name="field[{{ $field->id }}][0][lat]" 
            value="{{ $value[0]['lat'] ?? null }}">
            @includeWhen($errors->has("field.{$field->id}.0.lat"), 'icore::web.partials.errors', ['name' => "field.{$field->id}.0.lat"])
            <input type="hidden" id="field.{{ $field->id }}.0.long" name="field[{{ $field->id }}][0][long]" 
