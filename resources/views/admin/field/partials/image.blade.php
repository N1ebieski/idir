<div class="form-group">
    ...
    <div class="custom-file" id="field.{{ $field->id }}">
        <input 
            type="file" 
            class="custom-file-input {{ $isValid("field.{$field->id}") }}" 
            id="img" 
            name="field[{{ $field->id }}]"
    ...
    <div class="custom-control custom-checkbox">
        <input 
            ...
            id="delete_img.{{ $field->id }}"
            name="delete_img[{{ $field->id }}]" 
            {{ old("delete_img.{$field->id}") ? 'checked' : null }}
        >
        <label 
            class="custom-control-label" 
            for="delete_img.{{ $field->id }}"
        >
            {{ trans('icore::default.delete_img') }}
        </label>
        <input 
            type="hidden" 
            name="field[{{ $field->id }}]" 
